...
const DOC_EXTENSIONS = ['.pdf', '.psd', '.cdr', '.ai', '.doc', '.docx'];

function emptyCard(absoluteNumber) {
  return { label: `Target ${absoluteNumber}`, imageFile: null, imagePreviewUrl: null, docFile: null, docName: '' };
}
...
export default function DocumentSetupScreen({ onStart, onSignOut, isPublic }) {
  ...
  const canStart = cards.length > 0 && cards.every((c) => c.imageFile && c.docFile);
  ...
  const handleDocFile = useCallback((index, file) => {
    if (!file) return;
    if (!isDocFile(file)) { alert('Please upload a .pdf, .psd, .cdr, .ai, .doc, or .docx file.'); return; }
    if (file.size > 100 * 1024 * 1024) { alert('Document must be under 100 MB.'); return; }
    setCards((prev) => prev.map((card, i) => i === index ? { ...card, docFile: file, docName: file.name } : card));
  }, []);
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      const docBlobs = cards.map((c) => {
        const blob = new Blob([c.docFile], { type: c.docFile.type || 'application/octet-stream' });
        blob.name = c.docFile.name;
        return blob;
      });
      ...
      // Use locally-compiled data + local document object URLs instead of loadTargets()
      ...
      const arTargets = targetsMeta.map((meta, i) => ({
        ...
        videoUrl: '',
        targetType: 'document',
        urlLink: URL.createObjectURL(cards[i].docFile),
        fileName: meta.fileName,
        previewUrl: '', // server generates this; not available until the next loadTargets()
      }));
    ...
    <div style={{ ...styles.screen, background: colors.bg }}>
      ...
      <div style={styles.cardList}>
        {cards.map((card, index) => (
          <DocTargetCard
            ...
            onDocFile={(f) => handleDocFile(index, f)}
      ...
      <div style={styles.bottomBar}>
        ...
        {showValidation && !canStart && compileState === 'idle' && (
          <p style={styles.validationHint}>Each target needs a marker image and a document file.</p>
        )}
...
function DocTargetCard({ index, card, showValidation, onImageFile, onDocFile, onRemove }) {
  const imageMissing = showValidation && !card.imageFile;
  const docMissing = showValidation && !card.docFile;
  ...
  return (
    <div style={card_s.card}>
      ...
      <p style={{ ...card_s.label, marginTop: 16 }}>Document<span style={card_s.hint}> &mdash; .pdf, .psd, .cdr, .ai, .doc, or .docx — opened (PDF) or downloaded (others) when marker is scanned</span></p>
      <input
        ref={docInputRef}
        type="file"
        accept="*/*"
        ...
        onChange={(e) => {
          ...
          e.target.value = '';
          if (file && !/\.(pdf|psd|cdr|ai|doc|docx)$/i.test(file.name)) {
            window.alert('Please select a .pdf, .psd, .cdr, .ai, .doc, or .docx file.');
            return;
          }
          onDocFile(file);
        }}
      ...
      {showDocPicker && (
        <>
          <div style={picker_s.backdrop} onClick={() => setShowDocPicker(false)} />
          <div style={picker_s.sheet}>
            <div style={picker_s.handle} />
            <p style={picker_s.title}>Select Document</p>
            <button style={picker_s.optionBtn} onClick={() => docInputRef.current?.click()}>
              <span style={picker_s.icon}>📁</span>
              <div>
                <p style={picker_s.label}>Phone Storage / Internal Storage</p>
                <p style={picker_s.hint}>Browse your device storage for a .pdf, .psd, .cdr, .ai, .doc, or .docx file</p>
              </div>
            </button>
            <button style={picker_s.cancelBtn} onClick={() => setShowDocPicker(false)}>Cancel</button>
          </div>
        </>
      )}
      <div
        style={{ ...card_s.zone, ...(docMissing ? card_s.zoneError : {}), height: card.docName ? 'auto' : 80, padding: card.docName ? 8 : '0 16px' }}
        onClick={() => setShowDocPicker(true)}
        role="button" tabIndex={0}
        onKeyDown={(e) => e.key === 'Enter' && setShowDocPicker(true)}>
        {card.docName ? (
          <div style={card_s.previewRow}>
            <div style={card_s.modelIconBox}>📄</div>
            <div>
              <span style={card_s.fileName}>{card.docName}</span>
              <br /><span style={card_s.changeLink}>Tap to change</span>
            </div>
          </div>
        ) : (
          <div style={card_s.zoneContent}>
            <span style={card_s.zoneIcon}>📄</span>
            <span style={card_s.zoneText}>{docMissing ? 'Document required' : 'Tap to select .pdf / .psd / .cdr / .ai / .doc / .docx'}</span>
          </div>
        )}
      </div>
      {docMissing && <p style={card_s.fieldError}>Document file is required</p>}
    </div>
  );
}
