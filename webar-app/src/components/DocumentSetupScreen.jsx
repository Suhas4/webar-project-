...
function emptyCard(absoluteNumber, imageFile = null, imagePreviewUrl = null) {
  return { label: `Target ${absoluteNumber}`, imageFile, imagePreviewUrl, docFile: null, docName: '' };
}
...
export default function DocumentSetupScreen({ onStart, onBack, onSignOut, isPublic, sharedImageFile, sharedImagePreviewUrl, sharedLabel }) {
...
function DocTargetCard({ index, card, showValidation, onImageFile, onDocFile, onRemove }) {
  ...
  return (
    <div style={card_s.card}>
      ...
      <UploadDropZone
        title="Tap to upload"
        hint="PDF, PSD, CDR, AI, DOC, DOCX"
        error={docMissing ? 'Document required' : ''}
        preview={card.docName ? <span style={{ fontSize: 40 }}>📄</span> : null}
        fileName={card.docName}
