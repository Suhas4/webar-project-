export default function LoadingScreen({ status, errorMessage, onEdit, onCreateAccount }) {
  if (status === 'tracking') {
  ...
  return (
    <div style={{
      ...
      {status === 'ready'   && <ReadyState onEdit={onEdit} onCreateAccount={onCreateAccount} />}
...
function LoadingState() {
  return (
    <>
      <Spinner />
      <Card>
        <p style={styles.title}>Starting AR Scanner</p>
        <p style={styles.subtitle}>Please wait — camera is starting…</p>
      </Card>
    </>
  );
}

function ReadyState({ onEdit, onCreateAccount }) {
  return (
    <div style={{
      ...
      {/* Create Account button — shown to guest users while waiting for target */}
      {onCreateAccount && (
        <button onClick={onCreateAccount} style={{
          ...
          borderRadius: 20, color: '#00C9A7', fontSize: 12, fontWeight: 700,
          fontFamily: 'Outfit, sans-serif', padding: '7px 14px', cursor: 'pointer',
      ...
      <Card>
        <ScanIcon />
        <p style={styles.title}>Scanner Ready ✓</p>
        <p style={styles.subtitle}>Point camera at the photo you uploaded</p>
        <p style={{ ...styles.subtitle, fontSize: 12, marginTop: 4, color: 'rgba(0,201,167,0.7)' }}>
...
function Spinner() {
  return (
    <>
      <style>{`@keyframes webar-spin { to { transform: rotate(360deg); } }`}</style>
      <div style={{
        width: 52, height: 52,
        border: '3px solid rgba(0,201,167,0.2)',
        borderTopColor: '#00C9A7', borderRadius: '50%',
        animation: 'webar-spin 0.75s linear infinite',
      }} />
    </>
  );
}
